<script>
    ...
    const url = 'http://localhost:8000/view2/';
    let data = [];
    ...
    {#each data as column}
        <div class="flex items-center w-full px-4 py-10 bg-cover card">
            ...
            <div class="max-w-md card-body">
                <h2 class="card-title">Product Category {column.product_type}</h2> 
                <h2><b>Total Number of Products in Category: </b></h2><p>{column.number_of_products}</p> 
            </div>
            </div>
        </div>
    {/each}

    <button class="btn btn-wide btn-lg" on:click={() => push('/view1')}>Go Back</button> 
</body>